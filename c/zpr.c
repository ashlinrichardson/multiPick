...
/* From Mesa-2.2\src\glu\project.c Compute the inverse of a 4x4 matrix.
 * Contributed by scotter@lafn.org. NB OpenGL matrices are COLUMN major */
static void invertMatrix(const GLdouble *m, GLdouble *out ){
    #define MAT(m,r,c) (m)[(c)*4+(r)]  // shorthand converting standard (row, col) to index
   ...
   if(det == 0.0){
      ...
      tmp[0] *= invDet;  // compute rest of inverse
      tmp[1] *= invDet;
      tmp[2] *= invDet;
      tmp[3] *= invDet;
